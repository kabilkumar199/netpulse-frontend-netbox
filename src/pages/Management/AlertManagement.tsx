...
const AlertManagement: React.FC<AlertManagementProps> = ({ onClose }) => {
  ...
  const fetchAlarms = async () => {
    ...
    try {
      ...
      //   deviceId: '10.3.40.1'
      // };
      const apiData = await api.get<ApiResponseAlarm[]>(API_ENDPOINTS.GET_ALERTS_URL, {
        // params: params
      });
      const mappedAlarms: Alarm[] = apiData.map((apiAlarm: ApiResponseAlarm, index: number): Alarm => {
        let type: 'Chassis' | 'Interface' | 'System' = 'System';
        if (apiAlarm.Description.toLowerCase().includes('interface')) {
          type = 'Interface';
        } else if (apiAlarm.Description.toLowerCase().includes('chassis')) {
          type = 'Chassis';
        }
        const severity: 'Critical' | 'Major' | 'Minor' =
          apiAlarm.Severity === 'Major' ? 'Major' :
            apiAlarm.Severity === 'Critical' ? 'Critical' :
              apiAlarm.Severity === 'Minor' ? 'Minor' : 'Minor';
        const status: 'New' | 'Acknowledged' | 'Assigned' | 'Resolved' | 'Cleared' =
          apiAlarm.ack === '0' ? 'New' : 'Acknowledged';
        const timestamp = new Date(apiAlarm.Timestamp).toISOString();
              ...
              <div className="flex justify-between border-t pt-4">
                <div className="flex space-x-2">
                  <button
                    ...
                    className={`rounded px-4 py-2 text-sm ${selectedAlarm.status === 'New'
