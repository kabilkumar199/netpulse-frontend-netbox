...
export function transformLldpToG6(lldpData: LldpResponse): G6GraphData {
  ...
  Object.entries(lldpData.nodes).forEach(([ipAddress, nodeData]) => {
    ...
    // Determine node stroke color based on status (border around the icon)
    const strokeColor = status === "Reachable" ? "#52c41a" : "#ff4d4f";
  ...
  return `
    <div style="max-width: 300px;">
      ...
      <div style="font-size: 12px;">
        <p style="margin: 4px 0;"><strong>Product:</strong> ${
          systemInfo["Product Name"]
        }</p>
        <p style="margin: 4px 0;"><strong>Vendor:</strong> ${
          systemInfo["Vendor"]
        }</p>
        <p style="margin: 4px 0;"><strong>Platform:</strong> ${
          systemInfo["Platform Name"]
        }</p>
        <p style="margin: 4px 0;"><strong>Serial Number:</strong> ${
          systemInfo["Serial Number"]
        }</p>
        <p style="margin: 4px 0;"><strong>MAC:</strong> ${systemInfo["MAC"]}</p>
        <p style="margin: 4px 0;"><strong>Status:</strong> 
          <span style="color: ${
            device.status === "Reachable" ? "#52c41a" : "#ff4d4f"
          }">
            ${device.status}
          </span>
        </p>
